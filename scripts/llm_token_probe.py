...
# Реальный экспонат каталога (db/seed_fabergemuseum.sql, id=5). Карточка
# собирается тем же exhibit_facts(), что и в проде, — иначе вход разъедется.
EXHIBIT = {
    "name": "Пасхальное яйцо «Лавровое дерево»",
    "year_created": 1911,
    "master_name": None,
    "material": "Золото, Рубин, Алмазы-\"розы\", Жемчуг, Перья, Нефрит, Аметисты, Цитрины",
...
GROUNDING = exhibit_facts(EXHIBIT)
...
def _story_user(style: str = "engaging") -> str:
    return (
        ...
        f"{GROUNDING}\n"
        f"Стиль: {_STYLE_HINT[style]}."
    )


def _questions_user(n: int = 4) -> str:
    return (
        f"На основе данных об экспонате ({GROUNDING}) предложи {n} "
