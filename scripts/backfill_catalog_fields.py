...
чтобы сводка шла по залам. ``include_service=true`` стоит не ради конкретного зала, а чтобы
объём не зависел от сегодняшних значений ``halls.is_service``: скрипт должен обойти ВЕСЬ
каталог независимо от того, какие залы сейчас скрыты от посетителя (зал 1 «Парадная лестница»
этим релизом как раз перестал быть служебным — п. I-1 баг-репорта 31.08.2026). ``short_description``, ``material`` и
...
# ``provenance`` парсер тоже отдаёт, но колонок под них нет: они идут только в отчёт, чтобы
# заказчик глазом видел, ТУ ЛИ строку разобрал парсер. ``origin_place`` колонку получил
# 31.08.2026 (п. I-2, «Дата создания И МЕСТО»), но заполняет её ОТДЕЛЬНЫЙ скрипт
# scripts/backfill_exhibit_origin_place.py: у новой колонки своя поставка и свой отчёт музею,
# и место не должно теряться среди правок материалов и техник.
...
def fetch_records(ids: Sequence[int], limit: Optional[int]) -> List[dict]:
    ...
    ``include_service=true`` — чтобы объём не зависел от текущих значений ``halls.is_service``:
    экспонаты в служебных залах настоящие, и правка каталога не должна их пропускать из-за
    флага видимости. С ``--ids`` списочная выдача не нужна вовсе — карточка отдаёт все поля разом.
    """
    if ids:
        records = []
