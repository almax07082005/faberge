...
либо теряются, либо оседают в `material`. `year_created` остаётся и хранит
**нижнюю границу** периода (NULL у вековых датировок), так что сортировка и
фильтры по году продолжают работать. В `search_vector` новые поля не входят —
как и `material`: вектор покрывает прозу. Это не только продуктовое решение, но и
техническое ограничение — `ADD COLUMN IF NOT EXISTS` не меняет выражение уже
созданной GENERATED-колонки, подробности в шапке миграции.
...
на `year_created` / `dating` / `master_name` / `material` / `techniques`. Модуль
чистый — без БД и сети, тесты в `tests/test_catalog_line.py`.
